...
def weatherMessage(code):
    if code == 1000:
        ...
        return ("Watch the mist when driving")
...
def specialMessage(content):
    cyd.display.draw_text(10,200,"                           ",uni, cyd.DGREEN) 
    if content == "":
        cyd.display.draw_text(10,200," ** No Messages **",uni, cyd.DGREEN)
    else:
        cyd.display.draw_text(10,200,content,uni, cyd.DGREEN)

def countdown(active):
    if active == True:
        actual_time=gmtime()
        day = actual_time[2]
        targetDay = 25
        remaining = targetDay - day
        if remaining > 1:
            message = str(remaining) + " days until Christmas "
        elif remaining ==1:
...
def updateClock(posX,posY,correction):
    actual_time=gmtime()
    print(actual_time)
    mins = actual_time[4]
    if mins <=9:
...
print ("Setting time and clock changes")

setTime() # updates time and traps if no wifi 
# Use this setting when the clocks change
clock_change = 1
flag = False

setUpScreen(False) # Set to False to avoid loading animation
...
while True:
    if flag!=True:
        ...
        for count in range (1,20): #change this value to update the weather more frequently
            print (count)
            if flag == True:
                break
            x = 10
            y = 80
            updateClock(x,y,clock_change)
            countdown(False)
        ...
        while True:
            ...
            x=randint(0,180)
            y=randint(15,85)
            cyd.display.draw_image("image2_133x143.raw",x,y ,w=133,h=143)
            cyd.display.draw_text(x+25,y-15,"Week 15",uni,cyd.GREEN)
            x, y = cyd.touches()
            if x != 0 and y != 0:
                print ("Screen pressed")
                flag = not flag
